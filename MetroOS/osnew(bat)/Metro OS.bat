...
cls 
echo. 
echo             CODE CAN RUN WITH NOTIHNG
echo.
...
if "%choice%"=="devmode" goto devtoolc
if "%choice%"=="cdrom" goto cdrom
if "%choice%"=="shop" goto shoptop
...
set "root=%~dp0G\systemfile\data"
set "current_dir=%root%"
set "current_dir_display=%root%"
cls
echo ======================================================
echo                     FILE MANAGER 
echo ======================================================
echo Current Directory: %current_dir_display%
echo ------------------------------------------------------
echo [Folders:]
for /d %%D in ("%current_dir%\*") do echo %%~nxD
echo.
echo [Files:]
for %%i in ("%current_dir%\*") do if not "%%~nxi"=="desktop.ini" echo %%~nxi
echo ======================================================
echo Type: create (new file/folder), delete (remove) 
echo view (file content,  back (previous folder)
echo root (go to root) E to out
echo ======================================================
set /p action=Enter command: 
if /i "%action%"=="create" goto create
if /i "%action%"=="delete" goto delete
if /i "%action%"=="view" goto view_file
if /i "%action%"=="back" call :go_back && goto filemanager
if /i "%action%"=="root" set "current_dir=%root%" & set "current_dir_display=G" & goto filemanager
if /i "%action%"=="E" goto system



:go_back
for %%I in ("%current_dir%..") do set "current_dir=%%~fI"
for %%I in ("%current_dir%") do set "current_dir_display=%%~nxI"
exit /b

:create
set /p newname=Enter folder/file name: 
if "%newname:~-1%"=="." (type nul > "%current_dir%\%newname%") else (mkdir "%current_dir%\%newname%")
pause
goto filemanager

:delete
set /p deltarget=Enter file/folder to delete: 
if exist "%current_dir%\%deltarget%" (rmdir /s /q "%current_dir%\%deltarget%" 2>nul || del /f /q "%current_dir%\%deltarget%") && echo "%deltarget%" deleted!
pause
goto filemanager
...
dir /b "%current_dir%\*" 2>nul || echo No files available.
echo ======================================================
set /p filename=Enter the file name to view (or type "exit" to return): 


echo DEBUG: Filename entered = "%filename%"


if /i "%filename%"=="exit" goto filemanager


set "filepath=%current_dir%\%filename%"
...
if exist "%filepath%" (
    ...
    (type "%filepath%" || echo ERROR: Could not read file!) 2>debug.log

    echo.
    echo ======================================================
) else (
    echo ERROR: File not found!
    echo DEBUG: File "%filepath%" does not exist.
    pause
)
...
cls
echo ======================================================
echo                  WELCOME TO CD/DVD ROM 
echo ======================================================
echo. 
echo ======================================================
echo 1. open CD ROM
echo 2. delete CD ROM
echo 3. exit 
echo =======================================================
set /p choice=Enter your choice: 


if "%choice%"=="1" goto opencd
if "%choice%"=="2" goto deletecd
...
if not exist "%CDPath%" mkdir "%CDPath%" 

echo Available CD files:


dir /b "%CDPath%\*.txt" 2>nul || echo No CD Available

echo ======================================================
set /p filename=Enter the news file name (or type "exit" to return): 

if /i "%filename%"=="exit" goto cdrom


if exist "%CDPath%\%filename%" (
    cls
    echo ======================================================
    echo                          CD
    echo ======================================================
    type "%CDPath%\%filename%"
    echo.
    echo ======================================================
) else (
    echo Error: File not found!
...
echo                      DELETE CD ROM
echo ======================================================
echo This option can delete files or perform other actions.
